...
const tarballs = join(releaseDirectory, "tarballs");
mkdirSync(tarballs);
...
for (const directory of packageDirectories) {
  const result = spawnSync(
    "npm",
    ["pack", join(releaseDirectory, directory), "--json", "--pack-destination", tarballs],
    { encoding: "utf8" }
  );
  ...
  const manifest = JSON.parse(
    readFileSync(join(releaseDirectory, directory, "package.json"), "utf8")
  );
...
writeFileSync(
  join(releaseDirectory, "npm-pack-manifest.json"),
